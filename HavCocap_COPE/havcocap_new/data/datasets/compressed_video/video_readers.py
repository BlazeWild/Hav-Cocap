...
from havcocap_new.utils.profile import Timer
...
def read_frames_compressed_domain(
    ...
    try:
        ...
        full_frame_gop = [g for g in full_frame_gop if len(g) > min(1, resample_num_mv)]  # remove gop which do not contain any B/P-frame
        num_avail_gops = len(full_frame_gop)
        if num_avail_gops == 0:
            raise ValueError(f"No GOPs found in {video_path}")

        # Rule 1: Use np.linspace ONLY for the I-frames (The Anchors)
        # Select exactly resample_num_gop (e.g. 8) I-frames spread evenly
        gop_idxs = np.linspace(0, num_avail_gops - 1, num=resample_num_gop).astype(int)

        i_frame_gop = []
        mv_frame_gop = []
        for gop_idx in gop_idxs:
            gop = full_frame_gop[gop_idx]
            i_frame_gop.append(gop[0])
            
            # Rule 2: Use STRICTLY CONSECUTIVE sampling for P-frames (The Burst)
            p_frames = gop[1: 1 + resample_num_mv]
            
            # Pad if this GOP has fewer P-frames than required
            while len(p_frames) < resample_num_mv and len(p_frames) > 0:
                p_frames.append(p_frames[-1])
            if len(p_frames) == 0:
                p_frames = [gop[0]] * resample_num_mv  # fallback
            
            mv_frame_gop.append(p_frames)
        ...
        if with_residual:
            ...
            ret["input_mask_res"] = torch.ones((resample_num_gop, resample_num_mv), dtype=torch.bool)
