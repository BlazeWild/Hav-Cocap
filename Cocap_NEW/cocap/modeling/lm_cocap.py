...
#use adamw instad of BertAdam may cause worse performance, we will investigate it in the future
from torch.optim import AdamW
#also import new loss consig
from .loss import microcap_kendall_loss_cfg
...
class CoCapLM(pl.LightningModule):
    ...
    def configure_optimizers(self) -> OptimizerLRScheduler:
        ...
        loss_params = [p for n, p in self.loss.named_parameters() if p.requires_grad]

        optimizer_grouped_parameters = [
            {"params": decay_param, "weight_decay": 0.01}, # EXPLICITLY ADDED WEIGHT DECAY
            {"params": no_decay_pretrained_param, "weight_decay": 0.0, "lr": self.clip_lr},
            {"params": no_decay_not_pretrained_param, "weight_decay": 0.0},
            {"params": loss_params, "weight_decay": 0.0, "lr": self.lr} 
        ]
        ...
        optimizer = AdamW(
            ...
            lr=self.lr,
            eps=1e-6
            # Do NOT pass max_grad_norm here. AdamW doesn't accept it.
        )
...
cocap_lm_cfg = builds(
    ...
    loss=microcap_kendall_loss_cfg, # new kendall loss
