...
def read_frames_compressed_domain(
    ...
    try:
        ...
        # 1. ENFORCE CONSECUTIVE P-FRAMES
        # We completely bypass random sampling. We ALWAYS take the first 'resample_num_mv' P-frames natively.
        i_frame_gop = []
        mv_frame_gop = []
        res_frame_gop = []
        for gop_idx in range(len(full_frame_gop)):
            i_frame_gop.append(full_frame_gop[gop_idx][0]) # Index 0 is always the I-frame
            
            # Slicing from 1 to 1+15 guarantees we take strictly consecutive P-frames
        ...
        # 2. ENFORCE NP.LINSPACE FOR GOP SAMPLING
        # We uniformly space out our GOP selection across the entire video.
        total_gops = len(mv_frame_gop)
        if total_gops > resample_num_gop:
            # np.linspace perfectly spaces out the requested number of GOPs
            gop_idxs = np.linspace(0, total_gops - 1, resample_num_gop).astype(int)
        else:
            # If the video is shorter than our requested GOP count, just take all available GOPs 
            # (The padding logic further down will fill the missing GOPs with zeros)
            gop_idxs = np.arange(total_gops)

        # Apply the linspace indices
        i_frame_gop = [i_frame_gop[i] for i in gop_idxs]
        mv_frame_gop = [mv_frame_gop[i] for i in gop_idxs]
        res_frame_gop = [res_frame_gop[i] for i in gop_idxs]
        timer("sample")
        # stack iframe
