...
from havcocap_new.modules.bert import BertSelfEncoder, BertLMPredictionHead
...
from havcocap_new.modules.audio.beats_encoder import BEATsAudioEncoder
...
class CaptionHead(nn.Module):
    ...
    def forward(self, visual_output, input_ids, input_mask):
        ...
        has_audio = "feature_audio" in visual_output

        # 1. Build the token type IDs dynamically
        type_tensors = [
        ...
        if has_audio:
            # Assign type ID 3 for Audio tokens
            type_tensors.append(
                torch.full((visual_output["feature_audio"].size(0), visual_output["feature_audio"].size(1)),
                           fill_value=3, dtype=torch.long, device=visual_output["feature_audio"].device)
            )
        ...
        input_types = torch.concat(type_tensors, dim=1)

        # 2. Build the combined multimodal sequence
        output_tensors = [visual_output["feature_context"], visual_output["feature_action"]]
        if has_audio:
            output_tensors.append(visual_output["feature_audio"])
            
        combined_multimodal_output = torch.cat(output_tensors, dim=1)

        # 3. Build the attention mask
        input_mask = torch.concat(
            [
                torch.ones(size=(combined_multimodal_output.size(0), combined_multimodal_output.size(1)),
                           dtype=torch.long, device=combined_multimodal_output.device),
                input_mask
            ], dim=1
        )

        hidden = self.cap_sa_decoder.forward(combined_multimodal_output, input_ids, input_mask, input_types)
...
class CompressedVideoCaptioner(nn.Module):
    ...
    def __init__(
        ...
        self.audio_encoder = audio_encoder if audio_encoder is not None else (BEATsAudioEncoder() if use_audio else None)
    ...
    def forward(self, inputs: Dict[str, Union[Tensor, Dict[str, Tensor]]]):
        ...
        if "visual_output" not in inputs:
            ...
            if self.use_audio and self.audio_encoder is not None and "audio" in inputs:
                # Get the audio tensor
                audio_tensor = inputs["audio"] # [Batch, Max Samples (e.g. 640000)]
                
                # Forward to BEATs encoder
                audio_features = self.audio_encoder(audio_tensor)
                
                # Reshape by GOPs (e.g. 16 GOPs -> [Batch, 16, tokens_per_gop, 512])
                n_gop = iframe.shape[1]
                reshaped_audio = self.audio_encoder.reshape_for_gop(audio_features, num_gop=n_gop)
                
                # Prepare visual features similarly to match dimensions, or just concatenate horizontally
                # Assuming compressed_visual_features["feature_action"] is [Batch, GOP, dim]
                # we'll append the average-pooled audio features per GOP
                audio_pooled = reshaped_audio.mean(dim=2) # [Batch, GOP, 512]
                
                # Instead of concatenating, add it as a new feature
                compressed_visual_features["feature_audio"] = audio_pooled
