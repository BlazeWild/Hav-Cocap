...
class HavCocapDataset(IterableDataset):
    def __init__(self, dataset_root, split="train", max_words=77, tokenizer_path=None, blacklist_file=None):
        ...
        if blacklist_file and os.path.exists(blacklist_file):
            ...
            try:
                ...
                blacklist_set = set(os.path.normpath(os.path.abspath(p)) for p in blacklist)
                
                # Filter
                self.video_files = []
                for v in video_files:
                    if os.path.normpath(os.path.abspath(v)) not in blacklist_set:
