...
class HavCocapDataset(IterableDataset):
    ...
    def __iter__(self):
        ...
        if worker_info is not None:
            # Shard the dataset
            per_worker = int(math.ceil(len(video_files) / float(worker_info.num_workers)))
            expected_start = worker_info.id * per_worker
            expected_end = min(expected_start + per_worker, len(video_files))
            
            video_files = video_files[expected_start:expected_end]
            # print(f"Worker {worker_info.id} processing {len(video_files)} files.")
