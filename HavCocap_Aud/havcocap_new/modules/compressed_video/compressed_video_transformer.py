...
from havcocap_new.modules.clip.model import VisionTransformer, CrossResidualAttentionBlock, LayerNorm, CLIP
...
class CompressedVideoTransformer(nn.Module):
    ...
    def from_pretrained(
        ...
        input_resolution=image_resolution // 4,  # 224 // 4 = 56
        patch_size=motion_patch_size,            # 8
        width=vision_width // 4,                 # 768 // 4 = 192 (Extremely truncated)
        layers=motion_layers,                    # 2 layers
        heads=motion_heads,                      # 8 heads
        output_dim=embed_dim,
        ...
        residual_encoder = VisionTransformer(
            ...
            patch_size=residual_patch_size, #64 (massive, strided patches   )
            width=vision_width, # 768
            layers=residual_layers,   # 2 layers
            heads=residual_heads, #  8 heads
            output_dim=embed_dim, 
            in_channels=3 
        )
        action_encoder = ActionEncoder(
        width=embed_dim,                         # inherit projection dim (usually 512)
        layers=action_layers,                    # 1 layer
        heads=action_heads,                      # 8 heads
        n_bp=n_bp, n_bp_type=2
        )
        return cls(
